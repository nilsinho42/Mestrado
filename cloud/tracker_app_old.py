...
class DeepSORTTracker:
    ...
    def _match_tracks_detections(self, detections, track_indices, detection_indices=None):
        ...
        # Compute feature cost matrix if features are available
        ...
        logger.debug(f"Cost matrix before thresholding: min={np.min(cost_matrix):.4f}, max={np.max(cost_matrix):.4f}, threshold={self.max_iou_distance}")

        # Apply maximum distance threshold
        cost_matrix[cost_matrix > self.max_iou_distance] = float('inf')

        # Mask rows and columns that are all inf
        valid_row_mask = np.isfinite(cost_matrix).any(axis=1)
        valid_col_mask = np.isfinite(cost_matrix).any(axis=0)

        if not np.any(valid_row_mask) or not np.any(valid_col_mask):
            logger.warning("All cost values are above threshold. No matching possible.")
            return [], track_indices, detection_indices

        filtered_cost_matrix = cost_matrix[np.ix_(valid_row_mask, valid_col_mask)]

        if not np.all(np.isfinite(filtered_cost_matrix)):
            logger.warning(f"Filtered cost matrix contains NaN or Inf values, replacing with large values")
            # Replace NaN and Inf with a large finite value instead of raising an error
            filtered_cost_matrix = np.nan_to_num(filtered_cost_matrix, nan=1.0, posinf=1.0, neginf=1.0)
        ...
        matches = [
            (track_indices[np.where(valid_row_mask)[0][i]],
            detection_indices[np.where(valid_col_mask)[0][j]])
        ...
        unmatched_detections = [j for j in detection_indices if j not in matched_detections]
        
        logger.info(f"Matching results: {len(matches)} matches, {len(unmatched_tracks)} unmatched tracks, {len(unmatched_detections)} unmatched detections")
...
# Define a custom TimeoutError to avoid conflicts with built-in TimeoutError in Python 3.10+
        ...
        try:
            ...
            for i, det in enumerate(data.detections):
                ...
                if time.time() - overall_start_time > max_processing_time:
                    logger.warning(f"[{video_id}] Processing time limit exceeded. Aborting.")
                    break
                ...
                try:
                    ...
                    # Check for invalid values in the box
                    if any(not isinstance(v, (int, float)) or np.isnan(v) or np.isinf(v) for v in box):
                        logger.warning(f"[{video_id}] Box contains invalid values: {box}")
                        continue
                    
                    # [x1, y1, x2, y2]
                    # Validate that x2 > x1 and y2 > y1
                    if box[2] <= box[0] or box[3] <= box[1]:
                        # Invalid format, not increasng coordinates
                        logger.warning(f"[{video_id}] Invalid box coordinates: {box}, must have x2 > x1 and y2 > y1")
                    ...
                    if hasattr(detection, 'features') and detection.features is None:
                        try:
                            ...
                            logger.warning(f"[{video_id}] Error extracting features: {e}")
                            # Continue without features
                    
                    deepsort_detections.append(detection)
                    logger.debug(f"[{video_id}] Added Detection: class={detection.class_name}, confidence={detection.confidence:.2f}, bbox={box}")
                except Exception as e:
                    logger.warning(f"[{video_id}] Error processing detection {i}: {e}")
                    continue
            
            # Update tracker with detections (with timeout protection)
            try:
                # Set a reasonable timeout for the update function based on detection count
                update_timeout = min(5 + len(deepsort_detections) * 0.2, 10)  # Between 5-10 seconds based on detection count
                
                # Check again if we're exceeding max processing time
                if time.time() - overall_start_time > max_processing_time:
                ...
                try:
                    ...
                    logger.error(f"[{video_id}] Tracker update timed out after {update_timeout}s. Returning partial results.")
                    tracks = session["tracker"].tracks  # Use existing tracks
            except Exception as e:
